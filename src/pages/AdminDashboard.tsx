...
import { Shield, Users, Sparkles, Award, DollarSign, TrendingUp, FileText, Upload, RefreshCw, BarChart2, BookOpen, CreditCard, ClipboardCheck, Megaphone, Settings, Download, Search, Edit, Trash2, CheckCircle, XCircle, ExternalLink, Eye, Plus, ArrowLeft, Save, Tag, Send, Calendar, Route, Bell } from "lucide-react";
...
interface Props {
  ...
  onRevokeFacilitator: (email: string, name: string) => Promise<void> | void;
    ...
    <div className="section bg-white text-left min-h-[50rem]">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5 mb-8">
        {[
          ...
          { label: "Enrolments", val: enrollments.length.toString(), icon: Users, color: "text-lani-blue" },
      ...
      {tab === "overview" && (
        <div className="grid gap-6">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="text-sm font-bold text-lani-navy flex items-center gap-1.5 mb-6 uppercase tracking-wider"><BarChart2 size={16} className="text-lani-blue" />Enrolments by Theme</h3>
              <div className="h-56">
        ...
        <div>
          ...
          <div className="mb-6 rounded-xl border border-slate-200 bg-white p-5">
            <div className="mb-3 flex items-center gap-2">
              <Sparkles size={15} className="text-lani-gold" />
              <h3 className="text-sm font-bold text-lani-navy">Facilitators</h3>
              <span className="text-[10px] font-bold text-slate-400">{facilitators.length}</span>
            </div>
            {facilitators.length > 0 ? (
              <div className="grid gap-2 sm:grid-cols-2">
                {facilitators.filter(f => { const q = learnerSearch.toLowerCase(); return !q || f.fullName.toLowerCase().includes(q) || f.email.toLowerCase().includes(q); }).map(f => (
                  <div key={f.email} className="flex items-center justify-between gap-3 rounded-lg border border-slate-150 bg-slate-50 px-3 py-2.5">
                    ...
                    <button
                      onClick={() => { if (window.confirm(`Revoke facilitator access for ${f.fullName}? They will be moved back to a learner account.`)) onRevokeFacilitator(f.email, f.fullName); }}
                      className="shrink-0 inline-flex items-center gap-1 rounded-md border border-red-200 px-2.5 py-1 text-[11px] font-bold text-red-500 hover:bg-red-50 transition-colors"
                    >
              ...
              <p className="text-xs text-slate-400 py-3">No facilitators yet. People who sign up as a facilitator appear here.</p>
            )}
          </div>
          ...
          <div className="table-shell border border-slate-200">
            ...
            {enrollments.length === 0 && <div className="py-16 text-center"><Users className="mx-auto text-slate-300" size={44} /><h3 className="mt-4 text-base font-bold text-lani-navy">No Enrolments</h3></div>}
          </div>
        </div>
      )}
